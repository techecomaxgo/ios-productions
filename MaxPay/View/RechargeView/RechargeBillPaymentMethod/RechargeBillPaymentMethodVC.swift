                            ...
                            if let err = error {
                                ...
                                if let dataResp = data  {
                                    ...
                                    if isConnected == true {
                                        ...
                                        self?.rechargeModelView.RechargePayUSecondAPICall(amountStr: self!.priceStr, phoneStr: "7906037514", provider: "Airtel", location: "Delhi NCR", txnidnew: txnID, latitude: "77.391029", longitude:"28.535517", device_id: Common.shared.getDeviceID(), client_ip: Common.shared.getDeviceIP())
...
//                        if self?.rechargeModelView.PayUFirstForRecharge?.status == "success" {
//                            
//                            self?.rechargeModelView.RechargeModelApiCall(skeyStr: "AVJQIdwn79iR0zlP0iKNKumME", amountStr: self?.priceStr ?? 0, rechargenumberStr: Common.shared.userMobile_NUMBER ?? "", phoneStr: Common.shared.phoneNo ?? "", txnIdStr:self?.paymentWalletDeductVM.paymentDeductModel?.data?.txn_id ?? "", ViaStr: "swallet")
//                            ObserveRechargeModelViewApi()
//                            DispatchQueue.main.async {
