...
class WalletDetailsViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
   
    private let viewModels = LatestWalletTransactionViewModel()
    private let viewModel = WalletDetailsViewModel()
    private let viewModelTrans = TransactionsViewModel()
    ...
    override func viewDidLoad() {
        ...
        viewModel.onSuccess = { [weak self] balance in
            DispatchQueue.main.async {
                self?.balanceLabel.text = balance // Update the balanceLabel with the fetched balance
            }
        }
              ...
              // Fetch wallet details with skey and phone parameters
              viewModel.fetchWalletDetails(skey: skey, phone: Common.shared.phoneNo ?? "")
        

        viewModels.onSuccess = { [weak self] in
            DispatchQueue.main.async {
                self?.tableView.reloadData() // Reload table view on success
            }
        }
        ...
        viewModels.fetchLatestWalletTransaction(skey: skey)
        setupWalletView()
        setupTableView()
        
        print("createdAtDates",createdAt)
        print("updatedAt",updatedAt)
        
    }
    
    func fetchLatestWalletTransaction() {
            let urlString = "https://api.maxupi.in/api/v1/wallet/s-latest-wallet"
    ...
    func setupTableView() {
        ...
        func setupWalletView() {
                ...
                walletTitleLabel.text = "Secondary Wallet"
          ...
          //  balanceLabel.text = viewModel.balance
                ...
                NSLayoutConstraint.activate([
                    ...
                    walletNumberLabel.topAnchor.constraint(equalTo: walletTitleLabel.bottomAnchor, constant: 5),
                    walletNumberLabel.centerXAnchor.constraint(equalTo: walletView.centerXAnchor),
    
                    balanceLabel.topAnchor.constraint(equalTo: walletNumberLabel.bottomAnchor, constant: 5),
                    balanceLabel.centerXAnchor.constraint(equalTo: walletView.centerXAnchor)
                ])
            }
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return viewModels.transactions.count // Use the transactions count
    }
    ...
    // Action method to show sorting options
...
//                self.viewModelTrans.fetchTransactionsByDate(skey: skey, date: self.createdAt ?? "")
//                self.tableView.reloadData()
//            }
//        }))
//        
...
//                self.viewModelTrans.fetchOldWalletTransactions(skey: skey)
//                self.tableView.reloadData()
//            }
//        }))
//        
...
//                self.viewModelTrans.fetchTransactions(skey: skey, startDate: self.createdAt ?? "", endDate: self.updatedAt ?? "")
//                self.tableView.reloadData()
//            }
//        }))
//        
...
//    }
    ...
    @objc func showSortingOptions() {
        ...
        alertController.addAction(UIAlertAction(title: "By Date", style: .default, handler: { _ in
            ...
            // Perform the fetch and table reload on the main thread
            DispatchQueue.main.async {
                self.viewModels.fetchTransactionsByDate(skey: skey, date: self.createdAt ?? "")
                self.tableView.reloadData()
                self.latestHeader.setTitle("By Date", for: .normal)
            }
        }))
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
       ...
       // cell.sNoLabel.text = "\(indexPath.row + 1)"
        cell.sNoLabel.text = transaction.id
...
class TransactionCell: UITableViewCell {
    ...
    func setupCell() {
        ...
        // Additional cell appearance customization, if needed
    }
}
